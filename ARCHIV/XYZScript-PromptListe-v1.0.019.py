...
# Datum: 2025-04-05
# Uhrzeit: 23:45
# Dateispeichername: XYPScript-Promptliste-V1-19
...
def insert_linebreaks_with_separator(text, every):
    words = text.split(',')
    result = ''
    line_len = 0
...
def update_list():
    ...
    for entry in data:
        ...
        if ansicht_var.get() == "bearbeiten":
            ...
            prompt_entry.insert(0, insert_linebreaks_with_separator(entry['prompt'], 130).replace('\n', ' '))
            ...
            for word in prompt_words:
                ...
                display_word = word if len(word) <= 130 else word[:130] + "..."
                tk.Checkbutton(row, text=display_word, variable=word_var, bg="#1F1F1F", fg="#FFA500", selectcolor="#000000").pack(side="left", padx=1)
                tk.Label(row, text="|", bg="#1F1F1F", fg="#FFA500").pack(side="left")
            def copy_selected():
            ...
            bez_btn.configure(command=copy_selected)
