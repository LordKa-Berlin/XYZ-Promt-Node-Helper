...
# Datum: 2025-04-05
# Uhrzeit: 19:10
# Dateispeichername: XYPScript-Promptliste-V1-7
# Beschreibung: GUI zur Verwaltung von Prompt-Texten mit Bezeichnern.
# - Checkboxen vor jedem einzelnen Prompt-Wort
# - Einträge können in das Prompt-Feld geladen werden
# - Löschfunktion pro Zeile ergänzt mit Sicherheitsabfrage
# - Buttons zur Aktivierung/Deaktivierung aller Checkboxen integriert
...
def delete_entry(bezeichner, prompt):
    global save_required
    if messagebox.askyesno("Löschen", f"Eintrag '{bezeichner}' wirklich löschen?"):
        data[:] = [d for d in data if not (d['bezeichner'] == bezeichner and d['prompt'] == prompt)]
        changed_entries.append(f"GELÖSCHT: {bezeichner}: {prompt}")
        save_required = True
        update_list()
        status_var.set("Eintrag gelöscht – nicht gespeichert!")

# Checkbox-Funktionen aktivieren/deaktivieren
...
# Buttons dafür
tk.Button(filter_frame, text="Alle aktivieren", command=lambda: set_all_checkboxes(True), bg="#FFA500", fg="#000000").pack(side="right", padx=5)
tk.Button(filter_frame, text="Alle deaktivieren", command=lambda: set_all_checkboxes(False), bg="#FFA500", fg="#000000").pack(side="right")

# Canvas-Listendarstellung
...
def update_list():
    ...
    for item in data:
        ...
        if hit:
            ...
            tk.Button(row_frame, text="Löschen", bg="#1F1F1F", fg="red",
                      command=lambda b=item['bezeichner'], p=item['prompt']: delete_entry(b, p)).pack(side="right", padx=5)
