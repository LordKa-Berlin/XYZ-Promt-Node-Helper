...
def value_button_press(event, eid, idx, btn):
    # Setze Flag und plane Callback nach 2000ms (2 Sekunden)
...
def value_button_release(event, eid, idx, btn):
    # Wenn nach 2 Sekunden der Callback noch nicht ausgelöst wurde, führe normalen Single‑Click aus
    ...
    if getattr(btn, "long_press_triggered", False):
        btn.long_press_triggered = False
        return
    # Normale Single‑Click-Aktion: Toggle zwischen aktiv und deaktiviert
...
# Neuer Clear-Button hinter dem Filterfeld:
    ...
    for idx, entry in enumerate(data):
        ...
        if ansicht_var.get() == "bearbeiten":
            ...
            for i, val in enumerate(values):
                ...
                # Statt des bisherigen <Button-1> Bindings erfolgt jetzt ein Long-Press-Mechanismus:
