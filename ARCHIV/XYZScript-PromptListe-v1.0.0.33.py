...
# Im Anzeige‑Modus wird über die Buttons gearbeitet, im Bearbeiten‑Modus erscheinen Editierfelder.
...
# Wert‑Button (Anzeige):
# Single‑Click-Toggle: Wenn aktiv (0) → deaktiviert (1), wenn deaktiviert (1) → aktiv (0).
def value_single_click(eid, index, btn):
    ...
    if index < len(states):
        if states[index] == 0:
            ...
            states[index] = 0
        # Bei State 2 (zum Löschen markiert) bleibt der Single‑Click unverändert.
...
# Der Modus-Toggle-Button ersetzt die bisherigen Radiobuttons
