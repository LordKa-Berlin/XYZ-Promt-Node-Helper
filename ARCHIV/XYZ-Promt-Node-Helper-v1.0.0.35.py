...
BACKUP_DIR = "backup-XYPScriptPromptListe"
...
row_bez_state = {}
# Für die einzelnen Wert‑Buttons (in der gleichen Reihenfolge wie beim Split des Prompts)
row_value_states = {}
...
# Neuer Output-Modus für die Zwischenablage-Ausgabe: "comma" (Output 1) oder "line" (Output 2)
output_mode_var = tk.StringVar(value="comma")
# Label für Anzeige des aktiven Output-Modus (wird oben in der Statusleiste angezeigt)
...
edit_widgets = []   # Dictionaries mit Referenzen zu den Editier-Widgets
...
def create_backup():
    ...
    if os.path.exists(DATA_FILE):
        ...
        backup_filename = f"XYPScriptPromptListe.json-{now:%Y-%m-%d-%H-%M-%S}"
...
def cleanup_backups(backup_dir):
    ...
    for f in os.listdir(backup_dir):
        if f.startswith("XYPScriptPromptListe.json-"):
            full_path = os.path.join(backup_dir, f)
            files.append((full_path, os.path.getmtime(full_path)))
    files.sort(key=lambda x: x[1], reverse=True)
    # Die 10 neuesten Backups bleiben erhalten
    to_check = files[10:]
    now = time.time()
        ...
        if entry["id"] == eid:
            ...
            if active_values:
                if output_mode_var.get() == "comma":
                    ...
                    mode_info = "Output 1 (comma separated)"
                else:
                    result = "\n".join(active_values)
                    mode_info = "Output 2 (line by line)"
                pyperclip.copy(result)
                status_var.set(f"Inhalt von '{entry['bezeichner']}' kopiert ({mode_info})")
            break
...
def value_button_press(event, eid, idx, btn):
    ...
    def long_press_action():
        ...
        text = btn.cget("text")
        pyperclip.copy(text)
        status_var.set(f"Langzeit-Klick: '{text}' kopiert ({output_mode_status_var.get()})")
...
# Header (Top Bar) – zusätzlich Anzeige des aktuellen Output-Modus
...
# Filter, Modus-/Import- und Output-Modus (Frame 3)
second_frame = tk.Frame(root, bg="#1F1F1F")
second_frame.pack(fill="x", pady=5)
toggle_mode_btn = tk.Button(second_frame, text="Bearbeiten", command=toggle_mode, bg="#FFA500", fg="#000000")
...
tk.Button(second_frame, text="Clear", command=lambda: filter_var.set(""), bg="#FFA500", fg="#000000").pack(side="left", padx=5)
filter_var.trace_add("write", lambda *args: update_list())
# Neue Output-Modus Optionsfelder (mit Zeilenumbruch im Label)
radio1 = tk.Radiobutton(second_frame, text="Output 1 comma separated\n(e.g. Forge & Automatic1111)",
                        variable=output_mode_var, value="comma", bg="#1F1F1F", fg="#FFA500",
                        command=lambda: (update_output_mode_status(), update_list()))
radio1.pack(side="right", padx=5)
radio2 = tk.Radiobutton(second_frame, text="Output 2 Line by line\n(e.g. für ComfyUI-kompatible Nodes)",
                        variable=output_mode_var, value="line", bg="#1F1F1F", fg="#FFA500",
                        command=lambda: (update_output_mode_status(), update_list()))
radio2.pack(side="right", padx=5)
tk.Button(second_frame, text="Import CSV", command=import_csv_window, bg="#FFA500", fg="#000000").pack(side="right", padx=5)
