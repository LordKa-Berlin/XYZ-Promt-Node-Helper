...
def update_button_appearance(btn, state, is_bez=False):
    if state == 0:  # normal
        if is_bez:
            # Im Anzeige‑Modus: Bezeichner normal = grauer Hintergrund, orange Schrift
...
# Zeilenumbruchfunktion: Fügt nach ca. 130 Zeichen einen Zeilenumbruch ein
...
# Doppelklick: Markiert die Zeile zum Löschen, oder (bei erneutem Doppelklick) setzt sie zurück.
def bezeichner_double_click(event, eid):
    # Wenn bereits zum Löschen markiert, zurücksetzen:
    if row_bez_state.get(eid, 0) == 2:
        row_bez_state[eid] = 0
        # Setze alle zugehörigen Wert-Buttons auf aktiv (0)
        for entry in data:
            if entry["id"] == eid:
                values = [v.strip() for v in entry["prompt"].split(",") if v.strip()]
                row_value_states[eid] = [0]*len(values)
...
# Hier wird jede Zeile als Checkbutton, Bezeichner‑Entry (schwarz/orange) und Prompt‑Textfeld dargestellt.
def create_bearbeiten_row(row_frame, entry, idx):
    ...
    # Prompt‑Textfeld (mehrzeilig) – Zeilenumbruch bleibt erhalten
...
def update_toggle_button_text():
    # Falls alle Wert‑Buttons (in row_value_states) aktiv sind, soll der Button "alle deaktivieren" heißen;
    # sonst "alle aktivieren".
    all_active = True
    for states in row_value_states.values():
        if states and not all(st == 0 for st in states):
            all_active = False
...
tk.Radiobutton(second_frame, text="Anzeige", variable=ansicht_var, value="anzeige", bg="#1F1F1F", fg="#FFA500", command=lambda: (update_list(), update_toggle_button_text())).pack(side="left")
tk.Radiobutton(second_frame, text="Bearbeiten", variable=ansicht_var, value="bearbeiten", bg="#1F1F1F", fg="#FFA500", command=update_list).pack(side="left")
# Neuer Toggle‑Button für alle Werte (nur im Anzeige‑Modus)
...
def update_list():
    ...
    for idx, entry in enumerate(data):
        ...
        if ansicht_var.get() == "bearbeiten":
            ...
            # Hier: im Anzeige‑Modus erhält der Bezeichner‑Button seinen eigenen Look: normal = grau/orange
            ...
            # Für jeden Wert einen eigenen Button
    ...
    # Aktualisiere den Toggle‑Button (nur im Anzeige‑Modus)
