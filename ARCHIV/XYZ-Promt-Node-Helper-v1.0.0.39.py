...
# Label zur Anzeige des aktiven Output-Modus in der Statusleiste
...
def update_button_appearance(btn, state, is_bez=False):
    if state == 0:  # normal
        ...
        # Für Werte-Buttons: Schrift immer schwarz, auch wenn deaktiviert
        if is_bez:
            btn.config(bg="#1F1F1F", fg="orange")
        else:
            btn.config(bg="#1F1F1F", fg="#000000")
...
# Frame 3: Aufteilung in linken und rechten Bereich
...
# In left_frame: Modus- und Filter-Widgets
...
# In right_frame: Output-Options Radiobuttons und Import CSV Button (ganz rechts)
radio1 = tk.Radiobutton(right_frame, text="Output 1 comma separated\n(e.g. Forge & Automatic1111)",
...
radio2 = tk.Radiobutton(right_frame, text="Output 2 Line by line\n(e.g. für ComfyUI-kompatible Nodes)",
                        variable=output_mode_var, value="line", bg="#1F1F1F", fg="#FFA500",
                        command=lambda: (update_output_mode_status(), update_list()))
radio2.pack(side="left", padx=5)
tk.Button(right_frame, text="Import CSV", command=import_csv_window, bg="#FFA500", fg="#000000").pack(side="left", padx=5)
