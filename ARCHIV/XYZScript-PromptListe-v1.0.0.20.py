...
# Datum: 2025-04-06
# Uhrzeit: 13:20
# Dateispeichername: XYPScript-Promptliste-V1-20
...
def update_list():
    ...
    for index, entry in enumerate(data):
        ...
        # Closure mit korrektem Index
        def make_copy_fn(prompt=entry['prompt']):
            def inner():
                selected_words = [w.strip() for w in prompt.split(',') if w.strip()]
                pyperclip.copy(", ".join(selected_words))
                status_var.set("Prompt kopiert")
            return inner

        bez_btn = tk.Button(row, text=entry['bezeichner'], bg="#333333", fg="#FFA500", font=("Arial", 10, "bold"), command=make_copy_fn(entry['prompt']))
        bez_btn.pack(side="left", padx=(5, 10))

        wrapped_prompt = insert_linebreaks(entry['prompt'])
        tk.Label(row, text=wrapped_prompt, bg="#1F1F1F", fg="#FFA500", wraplength=1300, justify="left").pack(side="left")
