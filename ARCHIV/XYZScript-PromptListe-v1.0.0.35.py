...
# Variable für den Modus (Anzeige oder Bearbeiten)
...
def create_backup():
    ...
    if os.path.exists(DATA_FILE):
        ...
        # Status aktualisieren (An bestehendem Status anhängen)
...
def add_entry():
    ...
    if bez and prompt:
        # Verwende unique Bezeichner, falls bereits vorhanden
        bez = get_unique_bezeichner(bez)
        # Duplikatprüfung: gleiche Kombination bezeichner+prompt
        for entry in data:
...
def save_data():
    ...
    # Im Bearbeiten‑Modus werden zuerst alle Änderungen übernommen
    if ansicht_var.get() == "bearbeiten":
        for widget in edit_widgets:
            ...
            # Wende unique-Bezeichner-Logik auch beim Bearbeiten an
...
def get_unique_bezeichner_for_import(base):
    # Verwende die gleiche Logik wie get_unique_bezeichner
...
# Filter & Modus-/Import-Toggle (Frame 3)
