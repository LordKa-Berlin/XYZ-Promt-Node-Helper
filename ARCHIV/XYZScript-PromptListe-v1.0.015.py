...
# Datum: 2025-04-05
# Uhrzeit: 22:57
# Dateispeichername: XYPScript-Promptliste-V1-15
...
# Zweiter Frame mit Filter, Ansicht, Copy Master, Toggle
...
toggle_btn = tk.Button(second_frame, text="Alle abwählen", command=toggle_all, bg="#FFA500", fg="#000000")
toggle_btn.pack(side="right", padx=10)
copy_master_btn = tk.Button(second_frame, text="Copy Master", command=copy_master_prompt, bg="#FFA500", fg="#000000")
copy_master_btn.pack(side="right")
...
def update_list():
    ...
    max_prompt_len = 0
    for entry in data:
        if len(entry['prompt']) > max_prompt_len:
            max_prompt_len = len(entry['prompt'])
    width_px = min(max_prompt_len * 6 + 400, screen_width - 100)
    canvas.config(width=width_px)
    ...
    for i, entry in enumerate(data):
        ...
        prompt_wrapped = insert_linebreaks(entry['prompt'], 130)
        tk.Label(row, text=prompt_wrapped, bg="#1F1F1F", fg="#FFA500", justify="left", anchor="w", wraplength=width_px).pack(side="left")
